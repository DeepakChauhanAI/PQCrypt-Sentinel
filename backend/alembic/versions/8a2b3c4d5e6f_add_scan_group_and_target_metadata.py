...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["created_by"], ["users.id"], name="fk_scan_groups_created_by_users"),
    )

    op.add_column("scans", sa.Column("scan_group_id", sa.UUID(), nullable=True))
    op.add_column("scans", sa.Column("target_label", sa.String(length=255), nullable=True))
    op.add_column(
        "scans",
        sa.Column(
            "target_kind",
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_scans_scan_group_id_scan_groups", "scans", type_="foreignkey")
