...
async def check():
    ...
    async with AsyncSessionLocal() as session:
        s = (await session.execute(select(Scan).where(Scan.id == scan_id))).scalar_one_or_none()
        if not s:
            print("Scan not found")
            return
        print("=== SCAN ROW ===")
        print(f"  status={s.status}  started_at={s.started_at}  completed_at={s.completed_at}")
        print(f"  duration={s.duration_seconds}s  assets_found={s.assets_found}  findings_created={s.findings_created}")
        ...
        logs = (await session.execute(
            select(ScanLog).where(ScanLog.scan_id == scan_id).order_by(ScanLog.timestamp)
        )).scalars().all()
        print(f"\n=== LOGS ({len(logs)} rows) ===")
        for log in logs:
