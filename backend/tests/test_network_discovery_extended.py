...
class TestEnumerateDNSTargets:
    def test_a_records(self):
        ...
        with patch("dns.resolver.resolve") as mock_resolve:
            mock_resolve.side_effect = lambda d, rtype: [mock_answer] if rtype == "A" else (_ for _ in ()).throw(Exception("no answer"))
    ...
    def test_unsafe_ip_filtered(self):
        ...
        with patch("dns.resolver.resolve") as mock_resolve:
            mock_resolve.side_effect = lambda d, rtype: [mock_answer] if rtype == "A" else (_ for _ in ()).throw(Exception("no"))
            with patch("app.scanners.network_discovery._ip_is_safe", return_value=False):
...
class TestDiscoverTLSHosts:
    def test_nmap_not_found_falls_back(self):
        with patch("asyncio.create_subprocess_exec", side_effect=FileNotFoundError), \
             patch("app.scanners.network_discovery.discover_tls_hosts_fallback", new_callable=AsyncMock) as mock_fb:
    ...
    def test_nmap_success_with_hosts(self):
        ...
        with patch("asyncio.create_subprocess_exec", return_value=mock_proc), \
             patch("app.scanners.network_discovery.validate_ip"), \
    ...
    def test_nmap_filters_unsafe_ip(self):
        ...
        with patch("asyncio.create_subprocess_exec", return_value=mock_proc), \
             patch("app.scanners.network_discovery.validate_ip", side_effect=UnsafeTargetError("loopback")):
            result = asyncio.run(discover_tls_hosts("10.0.0.0/30"))
            assert result == []
    ...
    def test_nmap_ipv6_fallback(self):
        ...
        mock_proc.communicate = AsyncMock(return_value=(xml.encode(), b""))
        with patch("asyncio.create_subprocess_exec", return_value=mock_proc), \
             patch("app.scanners.network_discovery.validate_cidr"), \
             patch("app.scanners.network_discovery.validate_ip"), \
             patch("app.scanners.network_discovery.validate_port"):
    ...
    def test_nmap_unsafe_port_skipped(self):
        ...
        with patch("asyncio.create_subprocess_exec", return_value=mock_proc), \
             patch("app.scanners.network_discovery.validate_ip"), \
             patch("app.scanners.network_discovery.validate_port", side_effect=UnsafeTargetError("unsafe port")):
            result = asyncio.run(discover_tls_hosts("10.0.0.0/30"))
            assert result == []
...
class TestSafeResolve:
    def test_calls_build_safe_target_async(self):
        with patch("app.scanners.network_discovery.build_safe_target_async", new_callable=AsyncMock) as mock_build:
