...
class TestVaultScannerInit:
    ...
    def test_init_strips_trailing_slash(self):
        connector = VaultScannerConnector(
            "https://vault:8200/", "tok", mount_point="kv/", path="app/data/"
        )
        assert connector.vault_url == "https://vault:8200"
...
class TestVaultRequest:
    ...
    async def test_404_returns_empty(self):
        ...
        with patch("httpx.AsyncClient", return_value=mock_client):
            result = await connector._vault_request(
                "GET", "secret/metadata/nonexistent"
            )
            assert result == {}
...
class TestListKvPaths:
    ...
    async def test_success(self):
        connector = VaultScannerConnector(
            "https://vault:8200", "s.token", mount_point="secret"
        )
        with patch.object(
            connector, "_vault_request", new_callable=AsyncMock
        ) as mock_req:
...
class TestUpsertSecret:
    ...
    async def test_import_new_crypto_secret(self):
        ...
        with patch.object(
            connector, "_read_kv_entry", new_callable=AsyncMock
        ) as mock_read:
    ...
    async def test_partial_crypto_material_cert_only(self):
        connector = VaultScannerConnector(
            "https://vault:8200", "s.token", mount_point="kv"
        )
        session = AsyncMock()
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = None
...
class TestSync:
    ...
    async def test_sync_flat_keys(self):
        connector = VaultScannerConnector(
            "https://vault:8200", "s.token", mount_point="secret", path="app"
        )
        session = AsyncMock()

        with patch.object(
            connector, "_list_kv_paths", new_callable=AsyncMock
        ) as mock_list, patch.object(
            connector, "_upsert_secret", new_callable=AsyncMock
        ) as mock_upsert:
    ...
    async def test_sync_nested_keys(self):
        connector = VaultScannerConnector(
            "https://vault:8200", "s.token", mount_point="secret", path=""
        )
        session = AsyncMock()
        ...
        with patch.object(
            connector, "_list_kv_paths", side_effect=mock_list
        ), patch.object(
            connector, "_upsert_secret", new_callable=AsyncMock
        ) as mock_upsert:
            mock_upsert.return_value = "imported"
    ...
    async def test_sync_mixed_import_update(self):
        connector = VaultScannerConnector(
            "https://vault:8200", "s.token", mount_point="secret", path="data"
        )
        session = AsyncMock()
    ...
    async def test_sync_empty_keys(self):
        connector = VaultScannerConnector(
            "https://vault:8200", "s.token", mount_point="secret", path="empty"
        )
        session = AsyncMock()
