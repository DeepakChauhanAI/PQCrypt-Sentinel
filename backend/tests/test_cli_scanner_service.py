...
async def test_run_pqcscan(mock_subprocess):
    ...
    # Case 2: success
    with patch("shutil.which", return_value="/usr/bin/pqcscan"), patch(
        "app.services.cli_scanner_service.run_cli_tool", new_callable=AsyncMock
    ) as mock_run_tool:
...
async def test_run_ssh_audit(mock_subprocess):
    ...
    # Case 2: success
    with patch("shutil.which", return_value="/usr/bin/ssh-audit"), patch(
        "app.services.cli_scanner_service.run_cli_tool", new_callable=AsyncMock
    ) as mock_run_tool:
        ...
        mock_run_tool.return_value["raw_output"]["algorithms"]["kex"] = [
            "curve25519-sha256"
        ]
        res = await run_ssh_audit("127.0.0.1", 22)
...
async def test_run_testssl(mock_subprocess):
    ...
    with patch("shutil.which", return_value="/usr/bin/testssl.sh"), patch(
        "app.services.cli_scanner_service.run_cli_tool", new_callable=AsyncMock
    ) as mock_run_tool:
    ...
    mock_findings_list = [
        ...
        {
            "id": "cipher_hybrid-ml-kem",
            "severity": "INFO",
            "finding_id": "hybrid_ml_kem",
        },
        "invalid_entry",  # Cover line 199-200
    ]
    with patch("shutil.which", return_value="/usr/bin/testssl.sh"), patch(
        "app.services.cli_scanner_service.run_cli_tool", new_callable=AsyncMock
    ) as mock_run_tool, patch("builtins.open") as mock_open, patch(
        "os.remove"
    ) as mock_remove:
...
async def test_run_ike_scan(mock_subprocess):
    ...
    # Case 2: success
    with patch("shutil.which", return_value="/usr/bin/ike-scan"), patch(
        "app.services.cli_scanner_service.run_cli_tool", new_callable=AsyncMock
    ) as mock_run_tool:
        ...
        mock_run_tool.return_value = {"success": True, "stdout": stdout_data}
...
async def test_run_trivy(mock_subprocess):
    ...
    with patch("shutil.which", return_value="/usr/bin/trivy"), patch(
        ...
        mock_run_tool.return_value = {"success": True, "raw_output": trivy_list}
...
async def test_run_semgrep(mock_subprocess):
    ...
    with patch("shutil.which", return_value="/usr/bin/semgrep"), patch(
        ...
        mock_run_tool.return_value = {"success": True, "raw_output": semgrep_output}
