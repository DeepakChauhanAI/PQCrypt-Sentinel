...
async def test_k8s_get_credentials_dict():
    ...
    with patch(
        "app.connectors.k8s_connector.get_vault_secret", new_callable=AsyncMock
    ) as mock_get:
...
async def test_k8s_create_client_kubeconfig_tempfile():
    ...
    ), patch("os.path.exists", return_value=True) as mock_exists, patch(
        "os.unlink"
    ) as mock_unlink:
...
async def test_k8s_create_client_kubeconfig_path():
    connector = KubernetesConnector(
        credentials_ref={}, context="dev-ctx", kubeconfig_path="/path/kube.yaml"
    )
    ...
    with patch.object(
        ...
        mock_k8s_config.load_kube_config.assert_called_once_with(
            config_file="/path/kube.yaml", context="dev-ctx"
        )
...
async def test_k8s_get_secrets():
    ...
    mock_v1.list_secret_for_all_namespaces.return_value.items = [
        secret1,
        secret2,
        secret3,
    ]
...
async def test_k8s_get_certificates_api_error():
    ...
    mock_certs_v1 = MagicMock()
    mock_k8s_client.CertificatesV1Api.return_value = mock_certs_v1
    mock_certs_v1.list_certificate_signing_request.side_effect = Exception(
        "CSR API error"
    )
...
async def test_k8s_get_etcd_encryption_no_equals():
    ...
    c = MagicMock()
    c.command = ["kube-apiserver", "--encryption-provider-config", "--another-arg"]
...
async def test_k8s_get_apiserver_cert_no_equals():
    ...
    pod = MagicMock()
    c = MagicMock()
    c.command = ["kube-apiserver", "--tls-cert-file", "--tls-private-key-file"]
    pod.spec.containers = [c]
    mock_v1.list_namespaced_pod.return_value.items = [pod]
...
async def test_k8s_connector_sync_new_asset(mock_db):
    ...
    with patch.object(
        ...
        connector, "_get_secrets", new_callable=AsyncMock, return_value=fake_secrets
    ), patch.object(
        connector, "_get_certificates", new_callable=AsyncMock, return_value=fake_certs
    ), patch.object(
        connector,
        "_get_etcd_encryption",
        new_callable=AsyncMock,
        return_value=fake_etcd,
    ), patch.object(
        connector,
        "_get_apiserver_cert",
        new_callable=AsyncMock,
        return_value=fake_api_cert,
    ), patch.object(
        connector,
        "_get_kubelet_certs",
        new_callable=AsyncMock,
        return_value=fake_kubelets,
    ), patch(
        "app.scanners.cert_parser.parse_certificate"
    ) as mock_parse:
        ...
        assert added_asset.asset_metadata["cert_details"] == [
            {"subject": "CN=me", "namespace": "ns-1", "secret_name": "sec-1"}
        ]
...
async def test_k8s_connector_sync_existing_asset(mock_db):
    ...
    existing_asset = Asset(
        name="k8s:prod-cluster", asset_type="kubernetes", asset_metadata={}
    )
    mock_db_result = MagicMock()
    ...
    with patch.object(
        ...
        connector, "_get_secrets", new_callable=AsyncMock, return_value=[]
    ), patch.object(
        connector, "_get_certificates", new_callable=AsyncMock, return_value=[]
    ), patch.object(
        connector, "_get_etcd_encryption", new_callable=AsyncMock, return_value={}
    ), patch.object(
        connector, "_get_apiserver_cert", new_callable=AsyncMock, return_value={}
    ), patch.object(
...
async def test_k8s_connector_sync_client_creation_error():
    connector = KubernetesConnector(credentials_ref={})
    with patch.object(
        connector, "_create_k8s_client", side_effect=Exception("Failed to resolve host")
    ):
        with pytest.raises(RuntimeError) as exc:
...
async def test_k8s_connector_sync_parallel_exceptions(mock_db):
    ...
    with patch.object(
        ...
        connector, "_get_secrets", side_effect=Exception("sec error")
    ), patch.object(
        connector, "_get_certificates", side_effect=Exception("cert error")
    ), patch.object(
        connector, "_get_etcd_encryption", side_effect=Exception("etcd error")
    ), patch.object(
        connector, "_get_apiserver_cert", side_effect=Exception("api error")
    ), patch.object(
        connector, "_get_kubelet_certs", side_effect=Exception("kubelet error")
    ):
