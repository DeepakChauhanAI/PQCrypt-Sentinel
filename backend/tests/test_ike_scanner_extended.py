...
class TestDoSocketIKEProbe:
    ...
    def test_valid_response(self):
        ...
        sa_body = b"\x00\x00\x00\x08\x04\x00\x00\x13"  # DH group 19
        payload = (
...
class TestDoIKEProbe:
    ...
    def test_socket_fails_ike_scan_not_found(self):
        ...
        with patch(
            "app.scanners.ike_scanner._do_socket_ike_probe", return_value=fake
    ...
    def test_ike_scan_binary_timeout(self):
        ...
        with patch(
            "app.scanners.ike_scanner._do_socket_ike_probe", return_value=fake
        ), patch("shutil.which", return_value="/usr/bin/ike-scan"), patch(
            "subprocess.run", side_effect=subprocess.TimeoutExpired("ike-scan", 5)
        ):
            result = _do_ike_probe("10.0.0.1", 500, 5)
        assert result["success"] is False
        assert "timeout" in result["error_message"].lower()
    ...
    def test_import_subprocess_error(self):
        ...
        with patch(
            ...
            "builtins.__import__", side_effect=ImportError("no subprocess")
        ):
            result = _do_ike_probe("10.0.0.1", 500, 5)
        assert result["success"] is False
