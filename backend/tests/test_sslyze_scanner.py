...
def test_run_sslyze_sync_success(mock_parse_cert):
    ...
    with patch("sslyze.ServerNetworkLocation", return_value=mock_location), patch(
        "sslyze.server_connectivity.check_connectivity_to_server",
        return_value=mock_connectivity,
    ), patch("sslyze.ServerNetworkConfiguration"), patch(
        "sslyze.ServerScanRequest", return_value=mock_request
    ), patch(
        "sslyze.ScanCommand", MockScanCommand
    ), patch(
        "sslyze.Scanner"
    ) as mock_scanner_cls:
        ...
        assert (
            res["tls_versions"]["TLS_1_3_CIPHER_SUITES"]["accepted_ciphers"][0]["name"]
...
def test_run_sslyze_sync_connectivity_failure():
    mock_location = MagicMock()
    with patch("sslyze.ServerNetworkLocation", return_value=mock_location), patch(
        "sslyze.ServerNetworkConfiguration"
    ), patch(
        "sslyze.server_connectivity.check_connectivity_to_server",
...
@patch(
    "app.scanners.sslyze_scanner.parse_certificate",
...
async def test_scan_endpoint_with_sslyze_success():
    ...
    with patch(
        "app.scanners.sslyze_scanner._run_sslyze_sync", return_value=mock_result
    ):
...
async def test_scan_endpoint_with_sslyze_exception():
    with patch(
        "app.scanners.sslyze_scanner._run_sslyze_sync",
        side_effect=Exception("Unexpected executor error"),
    ):
        res = await scan_endpoint_with_sslyze("example.com", 443)
        assert res.success is False
