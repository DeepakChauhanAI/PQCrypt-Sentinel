...
async def test_oracle_get_credentials_variants():
    connector = OracleTDEConnector(credentials_ref={"vault_path": "sec", "version": 1}, host="10.0.0.3")
    with patch("app.connectors.tde_connector.get_vault_secret", new_callable=AsyncMock) as mock_vault:
...
async def test_oracle_sync_success_use_wallet(mock_db):
    ...
    ts_rows = [
        ("USERS", "ENCRYPTED", "AES256"),
        ("SYSTEM", "UNENCRYPTED", None)
    ]
    
    # 3. Master keys
    key_rows = [
        ("key-1", "1", "tag-1", "2026-01-01", "2026-01-02")
    ]
    
    # 4. Column encryption
    col_rows = [
        ("SCOTT", "EMP", "SAL", "AES192", "YES", "SHA-1")
    ]
...
async def test_oracle_sync_connection_failed(mock_db):
    connector = OracleTDEConnector(credentials_ref={"vault_path": "sec"}, host="oracle-host")
    mock_cx_oracle.connect.side_effect = Exception("Oracle service unavailable")

    with patch.object(connector, "_get_credentials", new_callable=AsyncMock, return_value={"username": "sys", "password": "pwd"}):
        res = await connector.sync(mock_db)
...
async def test_sqlserver_get_credentials_variants():
    connector = SQLServerTDEConnector(credentials_ref={"vault_path": "sec", "version": 1}, host="10.0.0.4")
    with patch("app.connectors.tde_connector.get_vault_secret", new_callable=AsyncMock) as mock_vault:
...
async def test_sqlserver_sync_success_new_asset(mock_db):
    ...
    db_keys_rows = [
        ("DB1", 3, "AES_256", 256, "CERTIFICATE", b"\xab\xcd", 100),  # Encrypted (Code 3)
    ...
    # 2. sys.certificates
    cert_rows = [
        ("TDECert", b"\x12\x34", "TDE Subject", "2030-01-01", "2026-01-01", "sa", "SOFTWARE_KEY")
    ]
    
    # 3. sys.symmetric_keys
    sym_rows = [
        ("SymKey1", "AES_256", 256, "guid-1234", "2026-01-01")
    ]
    
    # 4. sys.columns Always Encrypted columns
    col_rows = [
        ("dbo", "Employees", "SSN", "DETERMINISTIC", "AE_AES_256_CBC", "CMK1", "AZURE_KEY_VAULT")
    ]
...
async def test_sqlserver_sync_success_existing_asset(mock_db):
    ...
    db_keys_rows = [
        ("DB1", 1, "AES_256", 256, "CERTIFICATE", None, 0)
    ]
    mock_cursor.fetchall.side_effect = [db_keys_rows, [], [], []]

    existing_asset = Asset(
        name="sqlserver-tde:sql-host:1433",
        asset_type="database",
        asset_metadata={}
    )
    mock_db_result = MagicMock()
...
async def test_sqlserver_sync_connection_failed(mock_db):
    ...
    with patch.object(connector, "_get_credentials", new_callable=AsyncMock, return_value={"username": "sa", "password": "pwd"}):
        res = await connector.sync(mock_db)
        assert res["status"] == "error"
