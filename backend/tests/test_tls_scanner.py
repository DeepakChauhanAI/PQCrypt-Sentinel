...
async def test_scan_tls_endpoint_success():
    ...
    with patch("app.scanners.tls_scanner._do_tls_connect", return_value=mock_response) as mock_connect:
        res = await scan_tls_endpoint("test.example.com", port=443, timeout=5, verify_tls=True)
...
async def test_scan_tls_endpoint_exception():
    ...
    with patch("app.scanners.tls_scanner._do_tls_connect", side_effect=socket.timeout("Connection timed out")):
        res = await scan_tls_endpoint("test.example.com", port=443, timeout=2, verify_tls=True)
