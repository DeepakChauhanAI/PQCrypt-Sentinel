...
def test_ike_dh_group_31_34_classified_vulnerable():
    ...
    for g in ("31", "32", "33", "34", "35"):
        assert (
            _DH_GROUP_POLICY[g]["pqc_status"] == "vulnerable"
        ), f"Group {g} should be 'vulnerable'"
...
def test_cbom_dependency_types_use_valid_vocabulary():
    ...
    for child in new_dep["dependsOn"]:
        assert child["dependencyType"] in {
            "unknown",
            "required",
            "optional",
            "provided",
...
def test_cbom_protocol_component_falls_back_when_metadata_missing():
    ...
    proto = next(
        (c for c in out["components"] if c.get("bom-ref") == "protocol-abc"), None
    )
...
def test_cbom_protocol_component_uses_actual_tls_version():
    ...
    proto = next(
        (c for c in out["components"] if c.get("bom-ref") == "protocol-xyz"), None
    )
    assert proto is not None
...
def test_cbom_crypto_properties_field_order_follows_spec():
    ...
    for i, expected in enumerate(
        ["assetType", "certificateProperties", "implementationPlatform"]
    ):
        assert (
            keys[i] == expected
        ), f"expected {expected!r} at position {i}, got {keys[i]!r}"
...
def test_ssh_scanner_pqc_constants_pqc_kex_first():
    ...
    assert (
        first.startswith("sntrup")
        or first.startswith("mlkem")
        or first.startswith("kyber")
    )
...
def test_kmip_ssl_version_uses_module_constant():
    ...
    assert (
        'ssl_version="PROTOCOL_TLSv1_2"' not in src
    ), "string form still present in pkcs11_connector"
...
def test_cert_parser_disallowed_now_override():
    ...
    assert (
        "is_disallowed_now" in src
    ), "cert_parser must invoke is_disallowed_now for the override"
...
def test_generate_sarif_for_sast_findings_with_semgrep_results():
    ...
    assert any(
        "app/crypto.py" in str(r.get("locations", ""))
        or r.get("ruleId") == "weak-rsa-keygen"
        for r in results
    )
...
async def test_ssh_connector_locals_mutation_exception_handling():
    ...
    with patch.object(
        ...
        connector, "_enumerate_keystores", side_effect=ValueError("keystores crash")
    ), patch.object(
        connector,
        "_get_openssl_info",
        ...
        "_get_ssh_config",
        new_callable=AsyncMock,
        return_value={"Port": "22"},
    ), patch.object(
        connector,
        "_get_kerberos_config",
        new_callable=AsyncMock,
        return_value={"realm": "PQC.LOCAL"},
    ), patch(
        "paramiko.SSHClient", return_value=mock_client
    ):
...
async def test_k8s_connector_locals_mutation_exception_handling():
    ...
    with patch.object(
        connector, "_create_k8s_client", new_callable=AsyncMock
    ), patch.object(
        connector, "_get_secrets", side_effect=RuntimeError("secrets crash")
    ), patch.object(
        connector, "_get_certificates", new_callable=AsyncMock, return_value=[]
    ), patch.object(
        connector,
        "_get_etcd_encryption",
        new_callable=AsyncMock,
        ...
        connector, "_get_kubelet_certs", new_callable=AsyncMock, return_value=[]
    ):
...
async def test_winrm_connector_locals_mutation_exception_handling():
    ...
    with patch.object(
        ...
        "_get_credentials",
        new_callable=AsyncMock,
        return_value={"username": "a", "password": "b"},
    ), patch.object(
        connector, "_get_all_cert_stores", new_callable=AsyncMock, return_value={}
    ), patch.object(
        connector, "_get_cng_keys", side_effect=ValueError("cng crash")
    ), patch.object(
        ...
        connector, "_get_bitlocker_status", new_callable=AsyncMock, return_value={}
    ), patch.object(
        connector, "_get_firmware_info", new_callable=AsyncMock, return_value={}
    ), patch(
        "app.connectors.winrm_connector.WinRMConnector._run_ps_command",
        new_callable=AsyncMock,
    ):
...
async def test_k8s_connector_kubeconfig_cleanup():
    ...
    mock_k8s = MagicMock()
    with patch.dict(
        sys.modules, {"kubernetes": mock_k8s, "kubernetes.config": mock_k8s.config}
    ):
        from app.connectors.k8s_connector import KubernetesConnector
        ...
        with patch.object(
            ...
            "_get_credentials",
            new_callable=AsyncMock,
            return_value={"kubeconfig": fake_kubeconfig_content},
        ):
...
def test_cbom_asset_type_mappings():
    ...
    processed = json.loads(
        post_process_cbom(bom, {"cert-xyz": cert_obj, "algo-xyz": algo_obj})
    )
...
def test_celery_task_retry_on_exception():
    ...
    with patch(
        "celery.Task.request", new_callable=PropertyMock
    ) as mock_request_prop, patch.object(
        execute_scan, "retry", side_effect=Retry("retrying")
    ) as mock_retry, patch(
        "app.tasks.ScanOrchestrator"
    ) as mock_orchestrator:

        mock_request_prop.return_value = mock_request

        mock_orchestrator.return_value.run_scan.side_effect = RuntimeError(
            "DB OperationalError"
        )
...
def test_passive_ssh_capture():
    ...
    with patch("pyshark.LiveCapture", return_value=mock_live_cap), patch(
...
def test_saml_metadata_connector():
    ...
    assert (
        ...
        in added_asset.asset_metadata["bindings"]
    )
    assert (
        "urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress"
        in added_asset.asset_metadata["name_id_formats"]
    )
...
async def test_ssh_connector_tpm_probing():
    ...
    with patch.object(
        ...
        connector, "_run_ssh_command", side_effect=mock_run_cmd
    ), patch(
        "paramiko.SSHClient", return_value=mock_client
    ):
...
def test_execute_scheduled_scan():
    ...
    with patch("app.db.AsyncSessionLocal", return_value=mock_session_context), patch(
        "app.tasks.execute_scan.delay"
    ) as mock_delay, patch.dict("os.environ", {"PQC_PERIODIC_SCAN_TARGETS": ""}):
...
async def test_saml_metadata_ssrf_blocked():
    ...
    assert any(
        "safe IPs" in err or "loopback" in err.lower() or "ssrf" in err.lower()
...
async def test_ocsp_probe_dynamic_issuer_fetch():
    ...
    with patch("httpx.AsyncClient", return_value=mock_client):
        with patch(
            "cryptography.x509.ocsp.OCSPRequestBuilder.add_certificate"
        ) as mock_add:
            with patch(
                "app.scanners.ocsp_dnssec_scanner.build_safe_target_async",
