...
class TestPermissionDenied:
    ...
    def test_scan_sast_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/sast-direct", json={"target_path": "/src"})
        assert resp.status_code == 403

    def test_scan_kubernetes_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/kubernetes-direct", json={"context": "x"})
        assert resp.status_code == 403

    def test_scan_oracle_tde_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/oracle-tde-direct", json={
            "host": "o", "port": 1521, "service_name": "ORCL", "username": "u", "password": "p"
        })
        assert resp.status_code == 403

    def test_scan_sqlserver_tde_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/sqlserver-tde-direct", json={
            "host": "s", "port": 1433, "database": "m", "username": "u", "password": "p"
        })
        assert resp.status_code == 403

    def test_scan_pkcs11_hsm_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/pkcs11-hsm-direct", json={
            "library_path": "/lib.so", "pin": "1234"
        })
        assert resp.status_code == 403

    def test_scan_kmip_kms_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/kmip-kms-direct", json={"host": "k", "port": 5696})
        assert resp.status_code == 403

    def test_scan_adcs_ldap_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/adcs-ldap-direct", json={
            "domain_controller": "dc", "username": "u", "password": "p"
        })
        assert resp.status_code == 403

    def test_scan_jwt_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/jwt-direct", json={"tokens": ["a.b.c"]})
        assert resp.status_code == 403

    def test_scan_windows_cert_store_direct_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/scan/windows-cert-store-direct", json={
            "store_name": "My", "store_kind": "user", "dump": "x"
        })
        assert resp.status_code == 403

    def test_sync_pkcs11_hsm_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/sync/pkcs11-hsm", json={
            "library_path": "/lib.so", "credentials": {"vault_path": "sec/pkcs11"}
        })
        assert resp.status_code == 403

    def test_sync_kmip_kms_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/sync/kmip-kms", json={
            "host": "k", "port": 5696, "credentials": {"vault_path": "sec/kmip"}
        })
        assert resp.status_code == 403

    def test_sync_adcs_ldap_forbidden(self, viewer_client, mock_db):
        resp = client.post("/api/v1/connectors/sync/adcs-ldap", json={
            "domain_controller": "dc", "credentials": {"vault_path": "sec/adcs"}
        })
        assert resp.status_code == 403
...
class TestScanDirectErrorStatuses:
    ...
    def test_scan_windows_cert_store_direct_error_status(self, zero_findings_db, mock_db):
        with patch("app.connectors.winstore_connector.WindowsCertStoreConnector.sync", new_callable=AsyncMock) as mock_sync:
