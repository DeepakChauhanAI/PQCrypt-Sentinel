...
from app.tasks import _run_async, _set_worker_loop
...
class TestRunAsync:
    ...
    def test_runtime_error_event_loop_closed_recovered(self):
        ...
        with patch("app.tasks._worker_loop", mock_loop), patch(
            "app.tasks._set_worker_loop"
...
class TestExecuteReport:
    def test_report_not_found(self):
        ...
        with patch("app.db.AsyncSessionLocal", return_value=mock_session_cm), patch(
            "app.services.report_service.generate_report", new_callable=AsyncMock
        ) as mock_gen:
...
class TestExecuteScheduledScan:
    def test_env_targets_used(self):
        ...
        with patch(
            "app.db.AsyncSessionLocal", return_value=mock_session_cm
        ), patch.dict(
            os.environ, {"PQC_PERIODIC_SCAN_TARGETS": "10.0.0.1,10.0.0.2"}
        ), patch(
            "app.tasks.classify_target"
    ...
    def test_fallback_to_localhost(self):
        ...
        with patch(
            ...
            os.environ, {"PQC_PERIODIC_SCAN_TARGETS": ""}, clear=False
        ), patch(
            "app.tasks.classify_target"
        ) as mock_classify, patch(
            "app.tasks.execute_scan"
