...
def test_safe_target_defaults_deny_loopback():
    ...
    src = (
        ...
        .resolve()
        .parents[1]
        .joinpath("app", "scanners", "safe_target.py")
...
def test_safe_target_loopback_blocked_by_default():
    ...
    with patch.object(safe_target, "ALLOW_LOOPBACK", False), patch.object(
        safe_target, "ALLOW_LINK_LOCAL", False
    ), patch.object(safe_target, "ALLOW_PRIVATE_RANGES", False), patch.object(
        safe_target, "ALLOW_MULTICAST", False
    ):
...
def test_vault_helper_no_fallback_without_optin():
    ...
    env = {
        ...
        if k
        not in ("VAULT_URL", "VAULT_TOKEN", "VAULT_NAMESPACE", "ALLOW_ENV_FALLBACK")
    }
    ...
    with patch.dict(os.environ, env, clear=True):
        ...
        with patch.object(vault_helper.settings, "VAULT_URL", ""), patch.object(
            vault_helper.settings, "VAULT_TOKEN", ""
...
def test_cbom_pqcsafe_true_for_pqc_capable():
    ...
    assert (
        comp["cryptoProperties"]["algorithmProperties"]["nistQuantumSecurityLevel"] == 3
    )
