...
def test_dashboard_migration(mock_db):
    """The /migration endpoint computes migration and Mosca metrics and history."""
    call_count = {"n": 0}
    ...
    mock_db.execute.side_effect = _execute
    client = TestClient(app)
    resp = client.get("/api/v1/dashboard/migration")
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert "overall_pqc_transition_pct" in body
    assert body["overall_pqc_transition_pct"] == 66.7
    assert body["hybrid_deployment_pct"] == 33.3
    assert body["pqc_ready_count"] == 1
    assert body["safe_count"] == 0
    assert body["data_longevity_d"] == 10
    assert body["migration_duration_t"] == 2
    assert "estimated_deadline" in body
    assert "mosca_risk_level" in body
