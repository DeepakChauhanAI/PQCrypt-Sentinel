...
class SASTConnector(BaseConnector):
    ...
    GO_CRYPTO_PATTERNS = {
        ...
        "pqc_libs": [
            r"golang\.org/x/crypto/(kyber|dilithium|sphincs|falcon)",
            r"filippo\.io/.*pqc",
        ],
    }
    ...
    async def _log_event(
        self, level: str, phase: str, message: str, details: Optional[dict] = None
    ) -> None:
                                ...
                                findings.append(
                                    {
                                        ...
                                        "matched_text": match.group(0),
                                        "code_snippet": content[
                                            max(0, match.start() - 50) : match.end()
                                            + 50
                                        ].strip(),
                                        "language": language,
                                    }
    ...
    async def _run_semgrep(self) -> List[Dict[str, Any]]:
        if not shutil.which("semgrep"):
            await self._log_event(
                "info", "advanced", "semgrep not installed — skipping."
            )
            return []
        ...
        try:
            ...
            if proc.returncode in (0, 1):  # 1 = findings found
                ...
                await self._log_event(
                    "info",
                    "advanced",
                    f"semgrep finished — {len(findings)} findings.",
    ...
    async def _scan_container_files(self) -> List[Dict[str, Any]]:
        ...
        if self.target_path.exists():
            for root, dirs, files in os.walk(target):
                ...
                for fname in files:
                    ...
                    if (
                        "dockerfile" in name_lower
                        or name_lower == "containerfile"
                        or "docker-compose" in name_lower
                    ):
        ...
        await self._log_event(
            "info",
            "discovery",
            f"Scanning {len(container_files[:200])} container files...",
        )
        count = 0
            ...
            try:
                ...
                for proto in weak_protocols:
                    for match in re.finditer(
                        r"\b" + re.escape(proto) + r"\b", content, re.IGNORECASE
                    ):
                        line_no = content[: match.start()].count("\n") + 1
                        findings.append(
                            {
    ...
    async def _scan_kerberos_files(self) -> List[Dict[str, Any]]:
        ...
        if self.target_path.exists():
            for root, dirs, files in os.walk(target):
                ...
                for fname in files:
                    name_lower = fname.lower()
                    if (
                        name_lower == "krb5.conf"
                        or name_lower.endswith(".krb5")
                        or "kerberos" in name_lower
                    ):
        ...
        await self._log_event(
            "info",
            "discovery",
            f"Scanning {len(krb_files[:200])} Kerberos config files...",
        )
        count = 0
        for k_file in krb_files[:200]:
            ...
            try:
                ...
                for enctype in weak_enctypes:
                    for match in re.finditer(
                        r"\b" + re.escape(enctype) + r"\b", content, re.IGNORECASE
                    ):
                        line_no = content[: match.start()].count("\n") + 1
                        findings.append(
                            {
                ...
                for match in re.finditer(
                    r"(permitted_enctypes|default_tkt_enctypes|default_tgs_enctypes)\s*=\s*(.*)",
                    content,
                    re.IGNORECASE,
                ):
                    prop = match.group(1)
                    ...
                    line_no = content[: match.start()].count("\n") + 1
                    if not any(
                        modern in val.lower() for modern in ["aes256-cts", "aes128-cts"]
                    ):
                        findings.append(
                            {
                                "file": str(rel_path),
    ...
    async def sync(self, session: AsyncSession, **kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._log_event(
                "info",
                "discovery",
                f"Python scan complete — {len(python_findings) if not isinstance(python_findings, Exception) else 0} findings.",
            )
            ...
            await self._log_event(
                "info",
                "discovery",
                f"Java scan complete — {len(java_findings) if not isinstance(java_findings, Exception) else 0} findings.",
            )
            ...
            await self._log_event(
                "info",
                "discovery",
                f"Go scan complete — {len(go_findings) if not isinstance(go_findings, Exception) else 0} findings.",
            )

            # 4. JavaScript / TypeScript (React, Node, browser)
            await self._log_event(
                "info", "discovery", "Scanning JavaScript / TypeScript files..."
            )
            ...
            await self._log_event(
                "info",
                "discovery",
                f"JavaScript/TypeScript scan complete — {len(js_ts_findings) if not isinstance(js_ts_findings, Exception) else 0} findings.",
            )

            # 5. Manifests
            await self._log_event(
                "info", "discovery", "Scanning dependency manifests..."
            )
            manifest_findings = await self._run_phase(
            ...
            await self._log_event(
                "info",
                "discovery",
                f"Manifest scan complete — {len(manifest_findings) if not isinstance(manifest_findings, Exception) else 0} findings.",
            )

            # 5. Semgrep
            await self._log_event(
                "info", "advanced", "Running semgrep (if available)..."
            )
            ...
            await self._log_event(
                "info",
                "advanced",
                f"Semgrep scan complete — {len(semgrep_findings) if not isinstance(semgrep_findings, Exception) else 0} findings.",
            )

            # 6. Container files
            container_findings = await self._run_phase(
                "container", self._scan_container_files()
            )
            ...
            await self._log_event(
                "info",
                "discovery",
                f"Container scan complete — {len(container_findings) if not isinstance(container_findings, Exception) else 0} findings.",
            )

            # 7. Kerberos files
            kerberos_findings = await self._run_phase(
                "kerberos", self._scan_kerberos_files()
            )
            ...
            await self._log_event(
                "info",
                "discovery",
                f"Kerberos scan complete — {len(kerberos_findings) if not isinstance(kerberos_findings, Exception) else 0} findings.",
            )
            ...
            crypto_findings = [
                f for f in all_findings if f.get("category") != "vulnerable_dependency"
            ]
            dep_findings = [
                f for f in all_findings if f.get("category") == "vulnerable_dependency"
            ]
            ...
            await self._log_event(
                ...
                f"Source scan summary: {self._files_scanned} files examined, {len(all_findings)} total findings.",
                {
                    ...
                    "by_language": {
                        ...
                        "java": len(
                            [f for f in crypto_findings if f.get("language") == "java"]
                        ),
                        "go": len(
                            [f for f in crypto_findings if f.get("language") == "go"]
                        ),
                        "javascript": len(
                            [
                                f
                                for f in crypto_findings
                                if f.get("language") == "javascript"
                            ]
                        ),
                        "typescript": len(
                            [
            ...
            metadata = {
                ...
                "findings_by_language": {
                    "python": len(
                        [f for f in crypto_findings if f.get("language") == "python"]
                    ),
                    "java": len(
                        [f for f in crypto_findings if f.get("language") == "java"]
                    ),
                    ...
                    "docker": len(
                        [f for f in crypto_findings if f.get("language") == "docker"]
                    ),
                    "kerberos": len(
                        [f for f in crypto_findings if f.get("language") == "kerberos"]
                    ),
                },
            ...
            await session.execute(
                delete(Finding).where(
                    Finding.asset_id == asset.id, Finding.scan_id == scan_id
                )
            )
            ...
            from app.analysis.algo_classifier import (
                classify_algorithm as _classify_algorithm,
            )
            from sqlalchemy import delete as sa_delete

            await session.execute(
                sa_delete(Algorithm).where(
                    Algorithm.asset_id == asset.id, Algorithm.scan_id == scan_id
                )
            )
            ...
            for f in all_findings:
                ...
                if algo_key not in algo_map:
                    ...
                    algo_map[algo_key] = {
                        ...
                        "type": algo_type,
                        "pqc_status": pqc_status,
                        "is_quantum_vulnerable": cls_res.get(
                            "is_quantum_vulnerable", pqc_status == "vulnerable"
                        ),
                    }
