...
_VULNERABLE_ALGORITHMS = {
    "3des", "tripledes", "des", "rc4", "rc2", "blowfish",
    "md5", "sha1", "sha-1",
}
...
def classify_backup_algorithm(algorithm: Optional[str]) -> Dict[str, Any]:
    ...
    if "ml-kem" in norm or "kyber" in norm or "aes-256-gcm" in norm:
        return {"pqc_status": "pqc_ready", "name": algorithm or "unknown", "notes": "PQC or strong symmetric"}
    if any(v in norm for v in _VULNERABLE_ALGORITHMS):
        return {"pqc_status": "vulnerable", "name": algorithm or "unknown", "notes": "Legacy/weak algorithm"}
    if any(c in norm for c in _CLASSICAL_OK):
        return {"pqc_status": "vulnerable", "name": algorithm or "unknown", "notes": "Classically secure, not PQC"}
    return {"pqc_status": "unknown", "name": algorithm or "unknown", "notes": "Could not classify"}
    ...
    for idx, record in enumerate(records):
        try:
            ...
            asset = {
                ...
                "pqc_status": "vulnerable" if not encryption_enabled else classification["pqc_status"],
            }
            assets.append(asset)
...
def scan_veeam_config(config_text: str) -> Dict[str, Any]:
    ...
    encryption_enabled = bool(
        re.search(r"\bEnable\-?Encryption\s*[:=]\s*(true|yes|1)", config_text, re.IGNORECASE)
        or re.search(r"\bEncryption\s+Enabled\s*[:=]\s*(true|yes|1)", config_text, re.IGNORECASE)
    )
